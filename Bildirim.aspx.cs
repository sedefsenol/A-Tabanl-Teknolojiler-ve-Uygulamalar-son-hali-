...
namespace Cinescope
{
    public partial class Bildirim : System.Web.UI.Page
    {
        ...
        void BildirimleriGetir()
        {
            using (SqlConnection con = new SqlConnection(connStr))
            {
                SqlCommand cmd = new SqlCommand(@"
                    SELECT * FROM (
                        -- 1. ELEŞTİRİ ETKİLEŞİMLERİ (LIKE, REPLY)
                        SELECT 
                            ...
                            CASE 
                                ...
                                ELSE 'fa-bell'
                            END AS IconClass,
                            'Elestirisayfasi.aspx?filmId=' + CAST(E.FilmID AS NVARCHAR) + '&elestiriId=' + CAST(E.ID AS NVARCHAR) AS YonlendirmeUrl
                        FROM Bildirimler B
                        JOIN Kullanici K ON K.ID = B.GonderenKullaniciID
                        LEFT JOIN Elestiri E ON E.ID = B.IlgiliID
                        WHERE B.AlanKullaniciID = @uid AND B.BildirimTuru IN ('LIKE_REVIEW', 'REPLY_REVIEW')

                        UNION ALL

                        -- 2. MESAJ İSTEKLERİ (Okundu 1 olarak çekiyoruz ki pembe kalmasın)
                        SELECT 
                            1 AS Okundu, 
                        ...
                        -- 4. TREND FİLMLER (Son 24 saatte 10+ Yorum)
                        SELECT 
                            1 AS Okundu,
        ...
        void BildirimleriOkunduYap()
        {
            using (SqlConnection con = new SqlConnection(connStr))
            {
                SqlCommand cmd = new SqlCommand("UPDATE Bildirimler SET Okundu=1 WHERE AlanKullaniciID=@uid", con);
                cmd.Parameters.AddWithValue("@uid", Session["KullaniciID"]);
                con.Open();
                cmd.ExecuteNonQuery();
            }
        ...
        void OnerilenleriGetir()
        {
            ...
            using (SqlConnection con = new SqlConnection(connStr))
            {
                SqlCommand cmd = new SqlCommand(@"
                    ...
                    SELECT TOP 8
                        K.ID, K.KullaniciAdi, ISNULL(K.ProfilFoto, 'Gorsel/pp.png') AS ProfilFoto,
